...
def applyUpdate(content, ofn, qVerbose):
  with NamedTemporaryFile() as sfp:
    ...
    with sqlite3.connect(sfn) as uconn, \
      ...
      results = uconn.execute('SELECT * FROM tiles;').fetchall() # Get all the tile updates
      for result in results:
        (zoom, column, row, png) = result
        if qVerbose:
          print('Replacing ({},{},{},{}) in {}'.format(zoom,column,row,len(png),ofn))
...
def procDeletes(ofn, content, qVerbose):
  with sqlite3.connect(ofn) as conn:
    ...
    for item in a['deleted_tiles']:
      ...
      row = item['y']
      if qVerbose:
        print('Deleting zoom={}, column={}, and row={} from {}'.format(zoom, column, row, ofn))
...
action.add_argument('--update', action='store_true', help='Pull recent updates')
...
parser.add_argument('--outdir', default='MBTILES', help='where to write output to')
